  ...
  return (
    <StyledWrapper>
      ...
      <div className='container page'>
        ...
        <img src={main} alt='spot hunt' className='img main-img' />
      </div>
    </StyledWrapper>
  );
};
...
const Logo = () => {
  return <img src={logo} alt='spotify' className='logo' />;
};
...
const links = [
  { text: 'add spot', path: '.', icon: <FaWpforms /> },
  { text: 'all spots', path: 'all-spots', icon: <MdQueryStats /> },
...
- in a second, we will discuss why '.' in "add spot"
...
- create spots array where each item is an object with following properties
  id, company, position
...
app.post('/api/v1/spots', (req, res) => {
  ...
  const id = nanoid(10);
  // console.log(id);
  const spot = { id, company, position };
  spots.push(spot);
...
app.get('/api/v1/spots/:id', (req, res) => {
  ...
  if (!spot) {
    return res.status(404).json({ msg: `no spot with id ${id}` });
  }
  res.status(200).json({ spot });
});
...
app.patch('/api/v1/spots/:id', (req, res) => {
  const { company, position } = req.body;
  if (!company || !position) {
    return res.status(400).json({ msg: 'please provide company and position' });
  }
  ...
  res.status(200).json({ msg: 'spot modified', spot });
});

// DELETE SPOT

app.delete('/api/v1/spots/:id', (req, res) => {
  const { id } = req.params;
  const spot = spots.find((spot) => spot.id === id);
  if (!spot) {
    return res.status(404).json({ msg: `no spot with id ${id}` });
...
app.get('/api/v1/spots/:id', (req, res) => {
  ...
  const spot = spots.find((spot) => spot.id === id);
  if (!spot) {
    throw new Error('no spot with that id');
    return res.status(404).json({ msg: `no spot with id ${id}` });
  }
  res.status(200).json({ spot });
});
...
router.route('/:id').get(getSpot).patch(updateSpot).delete(deleteSpot);
...
export const createSpot = async (req, res) => {
  const { company, position } = req.body;
  const spot = await Spot.create({ company, position });
...
export const createSpot = async (req, res) => {
  const { company, position } = req.body;
  try {
    const spot = await Spot.create('something');
    res.status(201).json({ spot });
  } catch (error) {
...
export const deleteSpot = async (req, res) => {
  const { id } = req.params;
  const removedSpot = await Spot.findByIdAndDelete(id);

  if (!removedSpot) {
    return res.status(404).json({ msg: `no spot with id ${id}` });
  }
  res.status(200).json({ spot: removedSpot });
};
```
...
```js
res.status(StatusCodes.OK).json({ spots });
```
...
if (!spot) throw new NotFoundError(`no spot with id : ${id}`);
```
...
export const validateSpotInput = withValidationErrors([
  ...
  body('spotType').isIn(Object.values(SPOT_TYPE)).withMessage('invalid spot type'),
]);
```

```js
import { validateSpotInput } from '../middleware/validationMiddleware.js';

router.route('/').get(getAllSpots).post(validateSpotInput, createSpot);
router
  .route('/:id')
  .get(getSpot)
  .patch(validateSpotInput, updateSpot)
  .delete(deleteSpot);
```
...
- remove NotFoundError from getSpot, updateSpot, deleteSpot controllers
...
app.use('/api/v1/spots', authenticateUser, spotRouter);
```
...
export const getAllSpots = async (req, res) => {
  ...
  const spots = await Spot.find({ createdBy: req.user.userId });
  res.status(StatusCodes.OK).json({ spots });
};
```

#### Refactor Create Spot
...
export const getApplicationStats = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json({ users, spots });
};
```
...
```js
<div className='form-row'>
  <label htmlFor='spotStatus' className='form-label'>
    spot status
  </label>
  <select
    name='spotStatus'
  ...
  >
    {Object.values(SPOT_TYPE).map((itemValue) => {
      return (
...
- create SpotsContainer and SearchContainer (export)
...
const AllSpots = () => {
  ...
  return (
    <AllSpotsContext.Provider value={{ data }}>
      <SearchContainer />
      <SpotsContainer />
...
#### SpotsContainer - CSS (optional)

wrappers/SpotsContainer.js
...
#### SpotInfo - CSS (optional)

wrappers/SpotInfo.js
...
const EditSpot = () => {
  return <h1>EditSpot Page</h1>;
};
export default EditSpot;
```

- import EditSpot page
  App.jsx
...

{
  path: 'edit-spot/:id',
  element: <EditSpot />,
  loader: editSpotLoader,
  action: editSpotAction,
},
```
...
export const loader = async ({ params }) => {
  try {
    const { data } = await customFetch.get(`/spots/${params.id}`);
    return data;
  } catch (error) {
    toast.error(error.response.data.msg);
...
import { action as deleteSpotAction } from './pages/DeleteSpot';
 ...
 { path: 'delete-spot/:id', action: deleteSpotAction },
```
...
const Admin = () => {
  const { users, spots } = useLoaderData();
...
- setup in Register,Login, AddSpot, EditSpot, Profile
...
- add to updateUser, createSpot, updateSpot, deleteSpot
...
let stats = await Spot.aggregate([ ... ]); This line says we're going to perform an aggregation operation on the Spot collection in MongoDB and save the result in a variable called stats. The await keyword is used to wait for the operation to finish before continuing, as the operation is asynchronous (i.e., it runs in the background).

{ $match: { createdBy: new mongoose.Types.ObjectId(req.user.userId) } } This is the first stage of the pipeline. It filters the spots so that only the ones created by the user specified by req.user.userId are passed to the next stage. The new mongoose.Types.ObjectId(req.user.userId) part converts req.user.userId into an ObjectId (which is the format MongoDB uses for ids).

{ $group: { _id: '$spotStatus', count: { $sum: 1 } } } This is the second stage of the pipeline. It groups the remaining spots by their status (the spotStatus field). For each group, it calculates the count of spots by adding 1 for each spot ({ $sum: 1 }), and stores this in a field called count.
...
let monthlyApplications = await Spot.aggregate([ ... ]); This line indicates that an aggregation operation will be performed on the Spot collection in MongoDB. The result will be stored in the variable monthlyApplications. The await keyword ensures that the code waits for this operation to complete before proceeding, as it is an asynchronous operation.

{ $match: { createdBy: new mongoose.Types.ObjectId(req.user.userId) } } This is the first stage of the pipeline. It filters the spots to only those created by the user identified by req.user.userId.

{ $group: { _id: { year: { $year: '$createdAt' }, month: { $month: '$createdAt' } }, count: { $sum: 1 } } } This is the second stage of the pipeline. It groups the remaining spots based on the year and month when they were created. For each group, it calculates the count of spots by adding 1 for each spot in the group.
...
{ $limit: 6 } This is the fourth and last stage of the pipeline. It limits the output to the top 6 groups, after sorting. This is effectively getting the spot count for the last 6 months.

So, monthlyApplications will be an array with up to 6 elements, each representing the number of spots created by the user in a specific month and year. The array will be sorted by year and month, starting with the most recent.
...
export const loader = async () => {
  try {
    const response = await customFetch.get('/spots/stats');
    return response.data;
  } catch (error) {
    return error;
  }
...
const Stats = () => {
  const response = useQuery({
    queryKey: ['stats'],
    queryFn: () => customFetch.get('/spots/stats'),
  });
...
queryKey: ['stats'],: The queryKey property is an array that serves as a unique identifier for the query. In this case, the query key is set to ['stats'], indicating that this query is fetching statistics related to spots.

queryFn: () => customFetch.get('/spots/stats'),: The queryFn property specifies the function that will be executed when the query is triggered. In this case, it uses an arrow function that calls customFetch.get('/spots/stats'). The customFetch object is likely a custom wrapper around the fetch function or an external HTTP client library, used to make the actual API request to retrieve spot statistics.In React Query, the queryFn property expects a function that returns a promise. The promise should resolve with the data you want to fetch and store in the query cache.

customFetch.get('/spots/stats'): This line is making an HTTP GET request to the /spots/stats endpoint, which is the API route that provides the spot statistics data.
...
const statsQuery = {
  queryKey: ['stats'],
  queryFn: async () => {
    const response = await customFetch.get('/spots/statss');
    return response.data;
  },
};
